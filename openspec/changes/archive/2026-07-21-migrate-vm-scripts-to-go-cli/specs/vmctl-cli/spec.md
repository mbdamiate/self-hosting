...
`vmctl` SHALL expose `setup`, `cleanup`, and `backup` (with `snapshot`, `backup`, `restore`, and `list` sub-subcommands) as subcommands of one compiled binary, replacing `debian-vm-setup.sh`, `debian-vm-cleanup.sh`, and `debian-vm-backup.sh`.

#### Scenario: Invoking a subcommand
- **WHEN** a user runs `vmctl setup`, `vmctl cleanup`, or `vmctl backup <sub-subcommand>`
- **THEN** `vmctl` performs the equivalent behavior the corresponding bash script performed
