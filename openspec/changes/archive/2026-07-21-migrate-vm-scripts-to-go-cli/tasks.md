...
- [x] 7.4 Final end-to-end pass of `TESTING.md` against `vmctl` on a clean host — validated 2026-07-21 by reusing the same host right after its own `--purge-all` had removed the qemu/libvirt/kvm packages (confirmed via `dpkg -l` showing `rc` and `virsh` absent from `PATH`), then running `vmctl setup --name=clean-check-01` from scratch. Exercised, for the first time, every fresh-install code path unit tests and prior real-host testing never touched: real `apt-get install`, the libvirt `default` network being genuinely undefined (hit the documented `libvirt-nat-network-readiness` error path byte-for-byte identical to bash, then real first-time `net-start`/`net-autostart`), first-time `setfacl` on `$HOME`, and a real (uncached — confirmed no shared base-image cache exists) cloud image download. VM booted, got a DHCP IP, and SSH succeeded. **Deliberately not covered**: this host's user account was already a member of the `libvirt`/`kvm` groups from earlier testing, so the "first-time group grant, requires logout/login" messaging path was not exercised — stripping those groups would have forced a logout of the user's primary desktop session, judged disproportionate to the remaining risk. Accepted as a named residual gap.
